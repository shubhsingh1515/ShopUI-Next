...
const steps = [
  {
    ...
    description: "Define the team who will be supporting each client on their journey, such as care team members, family members, and AI agents.",
    features: [
    ...
    avatars: [
      { src: "https://ext.same-assets.com/3529801980/2499113889.png", color: "bg-zabit-primary" },
      { src: "https://www.zabit.com/_next/image?url=%2Fbusiness-images%2Ff1.png&w=256&q=75", color: "bg-zabit-primary" },
  ...
  },
  {
    number: 2,
    title: "Create a Plan",
    description: "Zabit imports a habit goal or home care plan using AI and breaks it down into a smart schedule for reminders and support.",
    features: [
  ...
  },
  {
    ...
    description: "Zabit uses ongoing multi-channel communication to help each client adhere to their care plan, leveraging the Support Circle to provide nudges, praise, and accountability.",
    features: [
      "Coordinates communications among all team members",
      "Builds accountability through social reinforcement"
    ],
    messages: [
      { from: "AI Agent", message: "Robert hasn't reported his meds today, give him a nudge!", color: "bg-purple-500" },
      { from: "Family", message: "Dad, reminder to take your meds today! ❤️", color: "bg-blue-500" },
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-xl text-gray-700 max-w-3xl mx-auto">
            ...
            <span className="font-semibold border-b-2 border-zabit-primary">collaborative</span>{' '}
            care plan adherence, empowering patients with a support system that works together.
          </p>
        </motion.div>
                ...
                <div className={`space-y-8 ${index % 2 === 1 ? 'lg:col-start-2' : ''}`}>
                  <div className="space-y-6">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-zabit-primary text-white rounded-full flex items-center justify-center text-xl font-bold">
                        {step.number}
                      </div>
        ...
        <div className="relative w-full h-full">
          {step.avatars?.slice(0, 3).map((avatar, index) => {
            ...
            return (
              ...
              >
                <div className={`w-14 h-14 ${avatar.color} rounded-full p-[2px] shadow-xl`}>
                  {avatar.src ? (
                    <Image
                      src={avatar.src}
                      alt="Avatar"
                    ...
                    >
                      ...
                      <div className="bg-white border-2 border-zabit-primary rounded-xl p-6 shadow-lg max-w-sm">
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <h4 className="font-bold text-zabit-dark">{step.schedule?.title}</h4>
                        </div>
                        <div className="space-y-3">
                          {step.schedule?.items.map((item, itemIndex) => (
                            <div key={itemIndex} className="flex items-center gap-3">
                              ...
                              <span className="ml-auto text-sm font-medium text-zabit-primary">{item.time}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                  ...
                  {step.number === 3 && (
                    ...
                    >
                      {step.messages?.map((message, messageIndex) => (
                        <motion.div
                          key={messageIndex}
                          initial={{ opacity: 0, x: messageIndex % 2 === 0 ? -20 : 20 }}
                          whileInView={{ opacity: 1, x: 0 }}
                          viewport={{ once: true }}
                          transition={{ duration: 0.6, delay: 0.5 + messageIndex * 0.2 }}
                          className={`flex items-start gap-2 ${messageIndex % 2 === 0 ? '' : 'flex-row-reverse'}`}
                        >
                          <div className={`w-8 h-8 ${message.color} rounded-full flex items-center justify-center flex-shrink-0`}>
                            <span className="text-white text-xs font-bold">
                              {message.from === 'AI Agent' ? 'AI' : message.from === 'Family' ? 'F' : 'D'}
                            </span>
                          </div>
                          <div className={`bg-gray-100 rounded-lg p-3 max-w-xs ${messageIndex % 2 === 0 ? 'rounded-tl-none' : 'rounded-tr-none'}`}>
